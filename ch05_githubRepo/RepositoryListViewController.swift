...
class RepositoryListViewController: UITableViewController {
  ...
  func fetchRepositories(of organization: String) {
    let t = Observable.from([organization])
      ...
      .map { objects in
        return objects.compactMap { dic -> Repository? in
          ...
          return Respository(id: id, name: name, description: description, stargazersCount: stargazersCount, language: language)
        }
      }
      .subscribe (onNext: { [weak self] newRepositories in 
        self?.respositories.onNext(newRepositories)
